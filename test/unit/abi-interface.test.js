 ...
 */
 ...
 describe("Interface", () => {
  ...
  it("getFunction resolves by name, signature, and 4-byte selector", async () => {
    ...
    assert.throws(() => iface.getFunction("0xdeadbeef"), (e) => e && e.code === "INVALID_ARGUMENT");
  });
  ...
  it("getFunction throws on ambiguous overloaded name; resolves by signature", async () => {
    ...
    const iface = new qc.Interface(abi);
    assert.throws(() => iface.getFunction("foo"), (e) => e && e.code === "INVALID_ARGUMENT");
  ...
  it("decodeFunctionData throws when the selector does not match", async () => {
    ...
    assert.throws(() => iface.decodeFunctionData("foo", data), (e) => e && e.code === "INVALID_ARGUMENT");
  });
  ...
  it("parseTransaction rejects unknown selector and short data", async () => {
    ...
    assert.throws(() => iface.parseTransaction({ data: "0xdeadbeef" }), (e) => e && e.code === "INVALID_ARGUMENT");
    assert.throws(() => iface.parseTransaction({ data: "0x00" }), (e) => e && e.code === "INVALID_ARGUMENT");
  });
  ...
  it("parseError decodes a custom error by selector", async () => {
    ...
    // The error selector equals keccak("InsufficientBalance(uint256,uint256)")[:4],
    // which is identical to the same-signature function selector - use that to build data.
    ...
    assert.throws(() => iface.parseError("0xdeadbeef"), (e) => e && e.code === "INVALID_ARGUMENT");
  });
  ...
  it("encodeDeploy encodes constructor args (0x when none)", async () => {
    ...
    // The constructor args encode identically to a same-typed function call minus its selector.
