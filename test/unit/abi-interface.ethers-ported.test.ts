 ...
 *   QuantumCoin's 32-byte addresses. TypeScript twin of
 *   abi-interface.ethers-ported.test.js.
 *
...
function A32(byteHex: string): string {
...
describe("ethers-ported: Interface type matrix (round-trip via calldata)", () => {
  function roundTrip(iface: any, name: string, args: any[]) {
  ...
  it("address array round-trip (32-byte)", async () => {
    ...
    assert.deepEqual(d.addrs.map((x: string) => x.toLowerCase()), a.map((x) => x.toLowerCase()));
  });
...
describe("ethers-ported: negative cases", () => {
  function frag(iface: any, name: string) {
  ...
  it("uint8 overflow throws", async () => {
    ...
    assert.throws(() => iface.encodeFunctionData(frag(iface, "f"), [256n]), (e: CodedError) => e && e.code === "INVALID_ARGUMENT");
    assert.throws(() => iface.encodeFunctionData(frag(iface, "f"), [-1n]), (e: CodedError) => e && e.code === "INVALID_ARGUMENT");
  ...
  it("int8 out-of-range throws", async () => {
    ...
    assert.throws(() => iface.encodeFunctionData(frag(iface, "f"), [128n]), (e: CodedError) => e && e.code === "INVALID_ARGUMENT");
    assert.throws(() => iface.encodeFunctionData(frag(iface, "f"), [-129n]), (e: CodedError) => e && e.code === "INVALID_ARGUMENT");
  ...
  it("bytes4 wrong length throws", async () => {
    ...
    assert.throws(() => iface.encodeFunctionData(frag(iface, "f"), ["0xdeadbeefaa"]), (e: CodedError) => e && e.code === "INVALID_ARGUMENT");
  ...
  it("wrong argument count throws", async () => {
    ...
    assert.throws(() => iface.encodeFunctionData(frag(iface, "transfer"), [A32("11")]), (e: CodedError) => e && e.code === "INVALID_ARGUMENT");
  });
  ...
  it("decoding truncated calldata throws", async () => {
    ...
    const truncated = data.slice(0, data.length - 8);
    assert.throws(() => iface.decodeFunctionData("transfer", truncated), (e: CodedError) => e && e.code === "INVALID_ARGUMENT");
  });
  ...
  it("parseTransaction with an unknown selector throws", async () => {
    ...
    assert.throws(() => iface.parseTransaction({ data: "0x12345678" }), (e: CodedError) => e && e.code === "INVALID_ARGUMENT");
  });
});
