...
// (and null->1).
const COMBOS: Array<{ keyType: number; signingContext: number | null; label: string }> = [
...
function tamperHexByte(hex: string, byteOffset: number): string {
...
describe("Message signing (EIP-191)", () => {
  ...
  describe("malformed signature inputs", () => {
    it("throws for empty, null, non-hex, and too-short signatures", () => {
      ...
      assert.throws(() => qc.verifyMessage(message, null as unknown as string));
  ...
  describe("message size limit", () => {
    ...
    const isInvalidArgument = (e: any) => e && e.code === "INVALID_ARGUMENT";
  ...
  describe("deterministic verification vectors", () => {
    ...
    const { vectors } = JSON.parse(fs.readFileSync(fixturePath, "utf8")) as {
      vectors: Array<{ keyType: number; signingContext: number | null; message: string; address: string; signature: string }>;
    };
