...
function compileAllSolidityTypes({ solPath }: { solPath: string }) {
  const [artifact] = compileSol({ solPaths: solPath, contractName: "AllSolidityTypes" });
...
describe("AllSolidityTypes (dynamic Contract)", () => {
  it("deploys and roundtrips all methods", async (t: { skip: (msg: string) => void }) => {
    ...
    const { abi, bytecode } = compileAllSolidityTypes({ solPath });
