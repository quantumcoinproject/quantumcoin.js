/**
 * Example: EIP-191 message signing (personal_sign) — offline, no blockchain required. TypeScript.
 * Run: npx tsx examples/sign-message.ts
 *
 * Demonstrates how to:
...
import { Wallet, hashMessage, verifyMessage } from "..";
import { logExample, logAddress } from "../test/verbose-logger";

async function signMessageExample(): Promise<void> {
  logExample("sign-message.ts", "starting");
  ...
  logExample("sign-message.ts", "done", { signatureLength: signature.length });
}
